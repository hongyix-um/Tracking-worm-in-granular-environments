...
def BBHP(filepath,param,train_img_count=-1,test_img_count=-1,want_train=None, want_test=None,dt_string=None,predict_path=None,batch_indices=None):
    ...
    if num_epochs==0 or not (predict_path is None):
        ...
        more_data = True
        #index = 0
        all_img_names = get_image_list(filepath + subfolder)
        #batch_size = 50
        ...
        #print(batch_indices)
        batch_img_names = [all_img_names[i] for i in batch_indices]
        batch_indices = np.array(batch_indices).astype(int)
        print(batch_indices)

        all_genHP, img_names, seg_masks = CreateGenerator(filepath=filepath,
                                                      subfolder=subfolder,
                                                      param=param,
                                                      want=batch_indices,
                                                      count=len(batch_img_names),
                                                      train_test_predict=train_test_predict,
                                                      chatter=False)
        all_genHP.normalize_images(norms['img_max'], norms['img_min'])

        SEG0, areas0 = Predict(param, modelHP, all_genHP, HP_output_folder, img_names,
                           true_pred=true_pred, true_pred_seg=true_pred_seg, seg_masks=seg_masks)
        print(f'[BB] -- Processed batch from {batch_img_names[0]} to {batch_img_names[-1]}')
